<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {  
        if(isset($_GET['idCarrito'])) {
            ...
            $pedido = new Pedidos();
            header("HTTP/1.1 200 OK");
            echo $pedido->newIdPedido($base->link);
            exit();
        } else {
            return false;
        }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'PUT') {
        if (isset($_GET['dniCliente'])) {
            $producto = new Carrito($_GET['idCarrito'], $_GET['dniCliente'], '', '');
            echo json_encode($producto->updateDni($base->link));
            header("HTTP/1.1 200 OK");
            exit();
        } else {
            $producto = new Carrito($_POST['idCarrito'], '', $_POST['idProducto'], $_POST['cantidad']);
            echo json_encode($producto->updateCantidad($base->link));
            header("HTTP/1.1 200 OK");
            exit();
        }
    }
